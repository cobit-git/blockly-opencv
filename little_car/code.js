...
function save_python() {
  ...
  var fileName = window.prompt('What would you like to name your file?', 'cobit_noname');
  if(fileName){
    var blob = new Blob([code], {type: 'text/plain'});
    saveAs(blob, fileName + ".py");
  }

  switch(haar_cascade_idx){
    case HAAR_NONE:
      console.log("haar none"); 
      break;
    case HAAR_FACE:
      var haar = new XMLHttpRequest();
      haar.open('GET', 'haarcascade_frontalface_default.xml');
      haar.onreadystatechange = function(){
       
      } 
      haar.send() 
      console.log("haar face!"); 
      var blob2 = new Blob([haar.response], {type: 'text/plain'});
      saveAs(blob2, "haarcascade_frontalface_default" + ".xml");
      break;
    case HAAR_PEDESTRIAN:
      var haar = new XMLHttpRequest();
      haar.open('GET', 'pedestrian.xml');
      haar.onreadystatechange = function(){
       
      } 
      haar.send()  
      console.log("haar pedestrian"); 
      var blob3 = new Blob([haar.response], {type: 'text/plain'});
      saveAs(blob3, "pedestrian" + ".xml");
      break;
    case HAAR_TRAFFIC_SIGNAL:
      var haar = new XMLHttpRequest();
      haar.open('GET', 'traffic_signal.xml');
      haar.onreadystatechange = function(){
        
      } 
      haar.send() 
      console.log("haar traffic signal"); 
      var blob4 = new Blob([haar.response], {type: 'text/plain'});
      saveAs(blob4, "traffic_signal" + ".xml");
      break;
    default:
      break;
  }
  haar_cascade_idx = HAAR_NONE;
